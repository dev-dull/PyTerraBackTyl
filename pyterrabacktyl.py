...
class PyTerraBackTYLException(Exception):
    ...
    LOCK_STATE_FOR_S_CHANGED_OUT_OF_PROCESS = (
        "Lock state for ENV %s appears to have been changed out-of-process!"
    )

    POST_PROCESSING_CLASS_SHOULD_BE_GOT_S = (
        "Expected list of strings for postprocessing class names. Got: %s"
    )
...
if C.POST_PROCESS_CLASSES:
    if isinstance(C.POST_PROCESS_CLASSES, str):
        ...
        raise PyTerraBackTYLException(
            PyTerraBackTYLException.POST_PROCESSING_CLASS_SHOULD_BE_GOT_S
            % type(C.POST_PROCESS_CLASSES)
        )
else:
...
def _set_lock_state(new_state, accepted_states, accepted_method, set_backend_state):
        ...
        if request.method == accepted_method:
            if _backends[_env][C.TYL_KEYWORD_BACKEND]._lock_state_ in accepted_states:
                ...
                if set_backend_state(json.loads(lock_text), raw=lock_text):
                    ...
                    return (
                        _json_string(
                            _backends[_env][C.TYL_KEYWORD_BACKEND].get_lock_state()
                        ),
                        C.HTTP_OK,
                    )
                _backends[_env][
                    C.TYL_KEYWORD_BACKEND
                ]._lock_state_ = old_state  # maintain the old/bad state.
            else:
                ...
                error_string = (
                    ...
                    + current_state
                    + " is the same as the requested lock state "
                    + new_state
                )
                if current_state == new_state:
                    ...
                    logging.debug(
                        "The requested state %s is not valid for the current state %s."
                        % (new_state, current_state)
                    )
                    return (
                        "The requested state is not valid for the current state.",
                        C.HTTP_CONFLICT,
                    )
    ...
    return (
        "A server error has occured, and the project's state could not be changed.",
...
def _run_post_processors():
    ...
    if request.method == C.HTTP_METHOD_LOCK:
        obj_funcs = [
            (pp, pp.on_locked) for pp in _backends[_env][C.TYL_KEYWORD_POST_PROCESSORS]
        ]
...
def tf_lock():
    set_env_from_url()
    if _allow_lock:
        state = _set_lock_state(
            C.LOCK_STATE_LOCKED,
            [C.LOCK_STATE_UNLOCKED],
            C.HTTP_METHOD_LOCK,
            _backends[_env][C.TYL_KEYWORD_BACKEND].set_locked,
        )
...
def tf_unlock():
    ...
    state = _set_lock_state(
        C.LOCK_STATE_UNLOCKED,
        [C.LOCK_STATE_LOCKED, C.LOCK_STATE_INIT],
        C.HTTP_METHOD_UNLOCK,
        _backends[_env][C.TYL_KEYWORD_BACKEND].set_unlocked,
    )
...
def tf_backend():
    ...
    if request.method == "POST":
        ...
        logging.info(
            "Stored new tfstate for ENV %s from IP %s." % (_env, request.remote_addr)
        )
        ...
        logging.info(
            "Fetched tfstate for ENV %s from IP %s." % (_env, request.remote_addr)
        )
...
def service_state():
    ...
    for env, backend in _backends.items():
        ...
        env_state = {
            ...
            C.TYL_KEYWORD_LOCK_STATE: loading
            or backend[C.TYL_KEYWORD_BACKEND]._lock_state_,
            C.TYL_KEYWORD_HTTP_STATE: loading
            or C.LOCK_STATES[backend[C.TYL_KEYWORD_BACKEND]._lock_state_],
            C.TYL_KEYWORD_BACKEND_STATUS: loading
            or backend[C.TYL_KEYWORD_BACKEND].backend_status(),
...
def shutdown():
    ...
    if request.remote_addr == "127.0.0.1":
        _allow_lock = False
        for env, backend in _backends.items():
            if backend[C.TYL_KEYWORD_BACKEND]._lock_state_ in [
                C.LOCK_STATE_INIT,
                C.LOCK_STATE_LOCKED,
            ]:
                _allow_lock = True
...
def set_env_from_url():
    ...
    if _env not in _backends:
        ...
        _backends[_env][C.TYL_KEYWORD_POST_PROCESSORS] = [
            c(_env, C) for c in post_process_classes
        ]
...
def _load_class(class_name, superclass):
    path_type = type(
        sys.path
    )  # Currently a list, but the Python overlords might change that some day.
    ...
    if _class.__name__ in [sc.__name__ for sc in superclass.__subclasses__()]:
        ...
        logging.error(
            PyTerraBackTYLException.S_IS_INVALID_SUBCLASS_TYPE % C.BACKEND_CLASS
        )
        raise PyTerraBackTYLException(
            PyTerraBackTYLException.S_IS_INVALID_SUBCLASS_TYPE % C.BACKEND_CLASS
        )
...
post_process_classes = [
    _load_class(c, abc_tylstore.TYLNonpersistent) for c in C.POST_PROCESS_CLASSES
]
...
if __name__ == "__main__":
    ...
    backend_service.run(
        host=C.BACKEND_SERVICE_IP, port=C.BACKEND_SERVICE_PORT, ssl_context=ssl_context
    )

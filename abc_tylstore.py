...
class TYLHelpers(object):
    ...
    def get_hostnames_from_tfstate(cls, json_obj):
        """
        :param json_obj: The tfstate (json) generated by terraform as an object (dict) or string (str).
        :return: assuming HELPER_HOSTNAME_QUERY_MAP is correctly configured in config.yaml for the Terraform provider
        used, a list (list) of hostnames contained in the tfstate file will be returned.
        """
        host_list = []

        if json_obj:
            # Importing modules within the function since this is the only place in this module where they're used;
            # no sense in taking up memory on modules unless we need them.
            import json

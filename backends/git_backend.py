...
class GitBackend(TYLPersistent):
    def __init__(self, environment, constants):
        ...
        self.ENV = (
            environment
            if environment
            else self.C.GIT_DEFAULT_CLONE_BRANCH.split("/")[-1]
        )
        ...
        self.working_dir = (
            getattr(self.C, "GIT_WORKING_PATH", tempfile.mkdtemp())
            or tempfile.mkdtemp()
        )
        ...
        self.tfstate_file_name = os.sep.join(
            [self.working_dir, self.C.TFSTATE_FILE_NAME]
        )
        self.lockfile = os.sep.join([self.working_dir, self.C.LOCK_STATE_LOCKED])
        self.logfile = os.sep.join(
            [self.working_dir, self.C.GIT_STATE_CHANGE_LOG_FILENAME]
        )

        self.repository = git.Git(self.working_dir)
        logging.info(
            "Git working directory for ENV %s: %s " % (self.ENV, self.working_dir)
        )
        self.__make_repo()
    ...
    def __make_repo(self):
        if not os.path.exists(self.working_dir):
            ...
            logging.info(
                "Cloned %s for ENV %s to directory %s"
                % (self.C.GIT_REPOSITORY, self.ENV, self.working_dir)
            )

        # TODO: Yuck. Why doesn't GitPython do this for me?
        branches = [
            b.split("/")[-1].strip() for b in self.repository.branch("-r").splitlines()
        ]
        ...
        try:
            ...
            logging.warning(
                'Could not properly suppress "N commits ahead of BRANCH" warnings which may result in a 500 error for the user.'
            )
    ...
    def _set_locked(self, state_obj, **kwargs):
        ...
        self.lock_commit_msg = self.C.GIT_COMMIT_MESSAGE_FORMAT.format(
            **defaultdict(str, state_obj)
        )
    ...
    def _set_unlocked(self, state_obj, **kwargs):
        ...
        if os.path.exists(self.lockfile):
            ...
            if state_obj:
                message = self.C.GIT_COMMIT_MESSAGE_FORMAT.format(
                    **defaultdict(str, state_obj)
                )
            else:
    ...
    def _update_log(self, json_obj, append=""):
        try:
            ...
            logging.error(
                "Scrollback value %s is not an integer. Using 300."
                % self.C.GIT_STATE_CHANGE_LOG_SCROLLBACK
            )
            scrollback = -300
        ...
        if json_obj:
            log_lines.append(
                self.C.GIT_STATE_CHANGE_LOG_FORMAT.format(**defaultdict(str, json_obj))
                + append
            )
        else:
            log_lines.append(
                "An out-of-process change was made using terraform (e.g. `terraform force-unlock`)"
            )
        foutin.write("\n".join(log_lines) + "\n")
        foutin.truncate()
        foutin.close()
    ...
    def _store_tfstate(self, state_obj, **kwargs):
        ...
        if os.path.exists(self.tfstate_file_name):
            logging.debug(
                "Renaming existing tfstate from %s to %s" % (self.lockfile, backup_file)
            )

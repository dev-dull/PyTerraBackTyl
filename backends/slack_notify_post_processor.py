...
class SlackNotifyPostProcessor(TYLNonpersistent):
    ...
    def on_locked(self, state_obj, **kwargs):
        ...
        if self.lock_info['Operation'] in self.C.SLACK_NOTIFICATIONS_ON_TF_ACTIONS:
            self._post_to_slack(self.C.SLACK_NOTIFY_FORMAT.format(**defaultdict(str, self.lock_info)), '*LOCKED* :lock:')
    ...
    def on_unlocked(self, state_obj, **kwargs):
        ...
        if self.lock_info['Operation'] in self.C.SLACK_NOTIFICATIONS_ON_TF_ACTIONS and self.C.SLACK_NOTIFY_UNLOCK:
            self._post_to_slack(self.C.SLACK_NOTIFY_UNLOCK_FORMAT.format(**defaultdict(str, self.lock_info)), '*Unlock* :unlock:')
        self.lock_info = {}
